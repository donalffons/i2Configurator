...
foreach ($files as $f) {
    ...
    $fileeditlink = '#file=' . pathinfo($_SERVER['PHP_SELF'])['dirname'] . '/WebGL Models/' . rawurlencode(FM_PATH) . '/' . rawurlencode($f);
...
<tr>
<?php if (!FM_READONLY): ?><td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td><?php endif; ?>
<td><div class="filename"><a href="<?php echo $filelink ?>" title="File info"><i class="<?php echo $img ?>"></i> <?php echo fm_convert_win($f) ?></a><?php echo ($is_link ? ' &rarr; <i>' . readlink($path . '/' . $f) . '</i>' : '') ?><?php echo (pathinfo($f, PATHINFO_EXTENSION) == "3djson" ? " <a href=\"editor.html".$fileeditlink."\">edit</a>" : "") ?></div></td>
...
<table class="table" id="main-table">
    <thead>
        <tr>
            <th colspan=5 style="text-align:center;">Variants</th>
        </tr>
        <tr>
            <th>Variant Name</th>
            <th colspan=3>Actions</th>
            <th>Files Referenced</th>
        </tr>
    </thead>
<?php
        $variants = $cdb->table('variants')->select("*")->fetch();
    ...
    <tr>
        ...
        <td>
            <a href="viewer.html?model=<?php echo rawurlencode(FM_PATH);?>&variant=<?php echo rawurlencode($variants[$i]["name"]);?>">view</a>
        </td>
        <td>
            <a href="editor.html?model=<?php echo rawurlencode(FM_PATH);?>&variant=<?php echo rawurlencode($variants[$i]["name"]);?>">edit</a>
        </td>
        <td>
            ...
            <script>
                $("a#delete").click(function(){
                    $.post(
                        "delete.php",
                        {
                            modelFolder: <?php echo "\"".rawurlencode(FM_PATH)."\""; ?>,
                            name: <?php echo "\"".$variants[$i]["name"]."\""; ?>
                        },
                        function( result )
                        {
        ...
        <td>
            <?php
                for ($j = 0; $j < count($variants[$i]["filenames"]); $j++) {
                    echo $variants[$i]["filenames"][$j];
                    if ($j < count($variants[$i]["filenames"])-1) {
                        echo ", ";
                    }
                }
            ?>
        ...
        <td colspan=5>
            ...
            <script>
                $("a#add").click(function(){
                    var variantname = prompt("Please enter variant name", "");
                    if(variantname == "") {
                        return;
                    }
                    $.post(
                        ...
                        function( result )
                        {
                            if (confirm("Do you want to edit the new variant now?")) {
                                window.location = "editor.html?model=<?php echo rawurlencode(FM_PATH);?>&variant="+encodeURIComponent(variantname);
                            } else {
                                window.location.reload(true);
                            }
                        }
